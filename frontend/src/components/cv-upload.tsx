...
export function CvUpload({ onFileUpload, isLoading = false }: CvUploadProps) {
  ...
  return (
    <div className="space-y-4">
      <div
        {...getRootProps()}
        className={`w-full rounded-lg border-2 border-dashed p-6 text-center transition-colors ${isDragActive ? 'border-primary bg-primary/10' : 'border-border'} ${isLoading ? 'cursor-not-allowed bg-muted/50' : 'cursor-pointer hover:border-primary/50'}`}
      >
        <input {...getInputProps()} />
      ...
      {isLoading && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <p className="text-sm font-medium text-muted-foreground">
            Analyzing candidate... this may take a moment.
          </p>
        </div>
      )}
    </div>
