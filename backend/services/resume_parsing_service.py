...
# Import the new, correct schemas for work and education history
...
def _calculate_total_experience(work_history: List[WorkExperienceCreate]) -> int:
    """
...
def _process_work_history_dates(
    work_history: List[WorkExperienceCreate],
...
async def process_resume_with_llm(
    ...
    and Application data. Updated to handle the new LLM output structure,
    manage the raw text locally, and include a fallback to the Gemini API.
    """
    havelsan_payload = _get_llm_payload(raw_text)

    # --- Primary Attempt: Havelsan API ---
    havelsan_retries = 3
    for i in range(havelsan_retries):
        try:
            ...
            response = requests.post(
                API_URL, headers=headers, json=havelsan_payload, timeout=10
            )
            ...
            if not llm_data:
                logger.error("JSON repair failed for Havelsan output.")
                break

            logger.info("Successfully parsed Havelsan content.")
            ...
            application = Application(
                ...
                resume_file_path=resume_file_url,
    ...
    # --- Fallback Attempt: Gemini API ---
    if GEMINI_API_KEY:
        gemini_payload = {
            "contents": [
                {
                    "parts": [
                        {"text": _get_llm_payload(raw_text)["messages"][1]["content"]}
                    ]
                }
            ],
            "generationConfig": {
        ...
        try:
            ...
            llm_data = _repair_llm_json(content_str)
            if not llm_data:
                logger.error("JSON repair failed for Gemini output.")
                return None, None

            logger.info("Successfully parsed Gemini content.")
